...
def check_product_discount_eligibility(item):
    ...
    if (
        discount.applies_to_type == DiscountApplicableType.Product.value
        and item["id"] in discount.applicable_ids
        and discount.is_currently_active
    ):
        applied, value = apply_discount(
            discount.discount_type, item["price"], discount.value
        )
