...
class SubscriptionService:
    ...
    def create(user_id, data):
        ...
        current_app.logger.info(f'TierType:  {TierType.BRONZE.name} {TierType.BRONZE.value}')
        bronze_tiers = MembershipTierDAO.get_active_by_tier(TierType.BRONZE.value)
        current_app.logger.info(f'bronze_tiers:  {[ tier.to_dict() for tier in bronze_tiers]} ')
        if len(bronze_tiers) > 0:
            ...
            current_app.logger.info(f"default_tier_id : {default_tier_id} benefits : {benefits} plan_details: {plan_details}")
            start_date , end_date = calculate_end_date_from_current(plan_details['duration_days'] )
