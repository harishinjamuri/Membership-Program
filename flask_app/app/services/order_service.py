...
from app.constants import OrderStatus, NotificationType, EntityType, TierType, BenefitType
    ...
    def confirm(user_id, order_id):
        ...
        for benefit in benefits:
            if benefit.benefit_type == BenefitType.FREE_SHIPPING:
                ...
                discount_amount = round(order.order_total * (benefit_value['percentage'] / 100), 2)
        ...
        if subscription.current_tier < tier.tier_level:
            SubscriptionService.update( subscription.subscription_id,  {"tier_id": TierType[tier.tier].value})
            NotificationService.create({
                "user_id": user_id,
            ...
            current_app.logger.info(f"Congratulations You have been upgraded to Tier {tier.tier}.",)
