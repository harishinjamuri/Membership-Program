...
class TierBenefitDAO:
    ...
    def get_all(is_active,page=1, per_page=10):
        items = TierBenefits.query.filter_by(is_active=True).order_by(TierBenefits.sort_order).all()
        return items
    ...
    def get_by_tier_id(tier_id):
        return TierBenefits.query.filter_by( tier_id=tier_id).order_by(TierBenefits.sort_order).all()
